...
function handleRequest(argv) {
  ...
  const request = {
    ...
    short_invocation: `review-hub slot --request ${shellQuote(requestRoot)} --model <MODEL_NAME>`
  };

  const requestDoc = buildRequestDoc(request);
  const promptTemplate = buildPromptTemplate(request, null);
  const rootManifest = {
  ...
  if (args.write) {
    ...
    writeText(path.join(requestRoot, "REQUEST.md"), requestDoc);
    writeText(path.join(requestRoot, "PROMPT.template.md"), promptTemplate);
    writeJson(path.join(requestRoot, "manifest.json"), rootManifest);
  }

  const models = ensureList(args.model);
  const slots = [];
  if (args.write && models.length) {
...
function handleInstallCommands(argv) {
  ...
  const repoRoot = resolvePath(args["repo-root"] || path.resolve(path.dirname(new URL(import.meta.url).pathname), ".."));
...
function buildPromptTemplate(request, modelName) {
...
function createReviewerSlot({ requestRoot, request, model }) {
  ...
  writeText(path.join(slot.slot_root, "PROMPT.md"), buildPromptTemplate(request, model));
